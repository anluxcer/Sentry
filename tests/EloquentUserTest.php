...
class EloquentUserTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testInGroup()
    {
        ...
        $user->shouldReceive('getGroups')->once()->andReturn([$group2]);
    ...
    {
        $group1 = m::mock('Cartalyst\Sentry\Groups\GroupInterface');
        $group1->shouldReceive('getPermissions')->once()->andReturn([
            'foo' => 1,
            'bar' => 1,
            'baz' => 1,
        ]);
        ...
        $group2 = m::mock('Cartalyst\Sentry\Groups\GroupInterface');
        $group2->shouldReceive('getPermissions')->once()->andReturn([
            'qux' => 1,
        ]);

        $user = m::mock('Cartalyst\Sentry\Users\Eloquent\User[getGroups,getPermissions]');
        $user->shouldReceive('getGroups')->once()->andReturn([$group1, $group2]);
    ...
    public function testHasAccess()
    {
        ...
        $user->shouldReceive('getMergedPermissions')->twice()->andReturn([
            'foo' => -1,
            'bar' => 1,
            'baz' => 1,
        ]);
    ...
    public function testHasAccessWithMultipleProperties()
    {
        ...
        $this->assertTrue($user->hasAccess(['bar', 'baz']));
        $this->assertFalse($user->hasAccess(['foo', 'bar', 'baz']));
    }
    ...
    public function testWildcardPermissionsCheck()
    {
        ...
        $user->shouldReceive('getMergedPermissions')->atLeast(1)->andReturn([
            'users.edit'   => 1,
            'users.delete' => 1,
        ]);
    ...
    public function testAnyPermissions()
    {
        ...
        $user->shouldReceive('getMergedPermissions')->once()->andReturn([
            'foo' => -1,
            'baz' => 1,
        ]);

        $this->assertTrue($user->hasAccess(['foo', 'baz'], false));
    }
    ...
    public function testAnyPermissionsWithInvalidPermissions()
    {
        ...
        $this->assertFalse($user->hasAccess(['foo', 'bar'], false));
    }

    public function testHasAnyAccess()
    {
        $user = m::mock('Cartalyst\Sentry\Users\Eloquent\User[hasAccess]');
        $user->shouldReceive('hasAccess')->with(['foo', 'bar'], false)->once()->andReturn(true);

        $this->assertTrue($user->hasAnyAccess(['foo', 'bar']));
    }
