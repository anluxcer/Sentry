...
class EloquentUserProviderTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $provider = m::mock('Cartalyst\Sentry\Users\Eloquent\Provider[createModel]', [$hasher = m::mock('Cartalyst\Sentry\Hashing\HasherInterface')]);
    ...
    {
        $provider = m::mock('Cartalyst\Sentry\Users\Eloquent\Provider[createModel]', [m::mock('Cartalyst\Sentry\Hashing\HasherInterface')]);
    ...
    public function testFindingByCredentialsFailsWhenModelIsNull()
    {
        ...
        $user->shouldReceive('getHashableAttributes')->once()->andReturn(['baz', 'bat']);
        $user->shouldReceive('hasGetMutator')->andReturn(false);

        $provider = m::mock('Cartalyst\Sentry\Users\Eloquent\Provider[createModel,getHashableCredentials]', [m::mock('Cartalyst\Sentry\Hashing\HasherInterface')]);
        $provider->shouldReceive('createModel')->once()->andReturn($user);

        $result = $provider->findByCredentials([
            'foo' => 'fooval',
            'bar' => 'barval',
            'baz' => 'unhashed_baz',
            'bat' => 'unhashed_bat',
    ...
    public function testFindingByCredentialsFailsForBadPassword()
    {
        ...
        $user->shouldReceive('getPasswordName')->once()->andReturn('password');
        $user->shouldReceive('newQuery')->andReturn($query);
        $user->shouldReceive('getHashableAttributes')->once()->andReturn(['password']);
        $user->shouldReceive('hasGetMutator')->andReturn(false);

        $provider = m::mock('Cartalyst\Sentry\Users\Eloquent\Provider[createModel,getHashableCredentials]', [$hasher]);
        $provider->shouldReceive('createModel')->once()->andReturn($user);
    ...
    {
        $provider = new Provider($hasher = m::mock('Cartalyst\Sentry\Hashing\HasherInterface'), 'UserModelStub1');
    ...
    public function testFindingAllUsers()
    {
        ...
        $collection->shouldReceive('all')->once()->andReturn([$user = m::mock('Cartalyst\Sentry\Users\User')]);

        $this->assertEquals([$user], $provider->findAll());
    }
    ...
    public function testFindingAllUsersInGroup()
    {
        ...
        $users->shouldReceive('get')->andReturn([$user1]);
        ...
        $this->assertEquals([$user1], $provider->findAllInGroup($group));
    }

    public function testFindingAllUsersWithAccess()
    {
        $provider = m::mock('Cartalyst\Sentry\Users\Eloquent\Provider[findAll]', [m::mock('Cartalyst\Sentry\Hashing\HasherInterface')]);

        $provider->shouldReceive('findAll')->once()->andReturn([
            $user1 = m::mock('Cartalyst\Sentry\Users\Eloquent\User'),
            $user2 = m::mock('Cartalyst\Sentry\Users\Eloquent\User'),
        ]);

        $user1->shouldReceive('hasAccess')->with($permissions = ['foo', 'bar'])->once()->andReturn(true);
        $user1->shouldReceive('hasGetMutator')->andReturn(false);
        $user2->shouldReceive('hasAccess')->with($permissions)->once()->andReturn(false);
        $user2->shouldReceive('hasGetMutator')->andReturn(false);

        $this->assertEquals([$user1], $provider->findAllWithAccess($permissions));
    }
    ...
    public function testFindingAllUsersWithAnyAccess()
    {
        ...
        $user1->shouldReceive('hasAnyAccess')->with($permissions = ['foo', 'bar'])->once()->andReturn(true);
        $user1->shouldReceive('hasGetMutator')->andReturn(false);
        $user2->shouldReceive('hasAnyAccess')->with($permissions)->once()->andReturn(false);
        $user2->shouldReceive('hasGetMutator')->andReturn(false);

        $this->assertEquals([$user1], $provider->findAllWithAnyAccess($permissions));
    }
}
